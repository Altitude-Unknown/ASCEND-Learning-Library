...
test('Filter combinations, empty state, clear, and shareable query', async({page})=>{
 ...
 await expect(page.locator('[data-resource]:visible')).toHaveCount(40);
...
test('Pages and resource links remain usable without JavaScript',async({browser})=>{
 ...
 await page.goto('http://127.0.0.1:8081/library/');await expect(page.locator('[data-resource]')).toHaveCount(40);
